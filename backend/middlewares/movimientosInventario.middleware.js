...
export const validarMovimiento = (req, res, next) => {
    ...
    if (
        id_camara_origen !== undefined &&
        id_camara_origen !== null &&
        isNaN(Number(id_camara_origen))
    ) {
        return res.status(400).json({
            error: 'El campo "id_camara_origen" debe ser numérico'
        });
    }
    if (
        id_camara_destino !== undefined &&
        id_camara_destino !== null &&
        isNaN(Number(id_camara_destino))
    ) {
        return res.status(400).json({
            error: 'El campo "id_camara_destino" debe ser numérico'
        });
    }
    // Regla mínima de coherencia por tipo de movimiento
    if (Number(tipo_movimiento) === 1 && !id_camara_destino) {
        return res.status(400).json({
            error: "Un ingreso a preenfrío (tipo=1) requiere id_camara_destino"
        });
    }
    if (Number(tipo_movimiento) === 2 && (!id_camara_origen || !id_camara_destino)) {
        return res.status(400).json({
